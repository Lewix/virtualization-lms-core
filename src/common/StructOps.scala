...
trait StructExp extends Structs with StructTags with BaseExp with EffectExp with VariablesExp with ObjectOpsExp with StringOpsExp with OverloadHack {
...
case _ => super.fresh
  ...
  def record_new[T : Manifest](fields: Seq[(String, Boolean, Rep[T] => Rep[_])]) = {
    ...
    struct(AnonTag(manifest[T].asInstanceOf[RefinedManifest[T]]), fieldSyms)
  }
